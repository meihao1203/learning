...
int recv_file(int socket_fd)
{
	...
	{
		bzero(&d,sizeof(d));
		recv(socket_fd,&d.len,sizeof(int),0);
		if(0==d.len)  //接收到对面发送的0就表示对端文件发送完毕了
		{
			break;
		}
		recv_n(socket_fd,d.buf,d.len);
		write(fd,d.buf,strlen(d.buf));
	}
...
int send_file(int scoket_fd,char *file_name)
{
	...
	{
		send(scoket_fd,&d.len,sizeof(int),0);
		send_n(scoket_fd,d.buf,d.len);
	}
	int endflag = 0;
	send(scoket_fd,&d.len,sizeof(int),0);  //文件发送完了就发送一个0，对面读到就知道 这边文件已经发送结束
